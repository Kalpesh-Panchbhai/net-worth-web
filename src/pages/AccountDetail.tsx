...
interface TxnRow {
  txn: Transaction;
  dt: ReturnType<typeof parseTxnDate>;
  delta: number;
      ...
      >
        <Box sx={{ display: "flex", alignItems: "flex-start", gap: 1.5, mb: 1.5 }}>
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            <Typography variant="caption" sx={{ color: cardMuted }}>{h.symbol}{h.unitsAreShares ? ` · ${fmtUnits(h.units)} units` : ""}</Typography>
          </Box>
          <Stack direction="row" spacing={0.5} alignItems="center" onClick={e => e.stopPropagation()} sx={{ ml: 1 }}>
            <XirrBadge value={h.xirr} size="sm" />
            {canDelete && (<IconButton size="small" onClick={() => onDelete(h)} sx={{ color: colors.error, opacity: 0.6, "&:hover": { opacity: 1, bgcolor: alpha(colors.error, 0.08) } }}>
        ...
        <Box sx={{ px: 1.5, py: 1, borderRadius: 1.5, bgcolor: cardSubtle, display: "inline-block", mb: 1.5, alignSelf: "flex-start" }}>
          ...
          <Typography sx={{ fontSize: "1.25rem", fontWeight: 750, letterSpacing: "-0.02em" }}>
            {fmt(h.currentDayValue, h.displayCurrency)}
          </Typography>
        </Box>
        ...
        <Stack direction="row" sx={{ gap: { xs: 0.75, sm: 1 }, flexWrap: "wrap", mt: "auto" }}>
          <Box sx={{ flex: 1, minWidth: { xs: 70, sm: 80 }, p: 1, borderRadius: 1.5, bgcolor: alpha(cardInvested, isDark ? 0.1 : 0.06), overflow: "hidden" }}>
            ...
            <Typography noWrap sx={{ fontSize: "0.8rem", fontWeight: 700, color: cardInvested }}>
              {fmt(h.invested, h.displayCurrency)}
            </Typography>
          </Box>
          {h.invested > 0 && (
            <Box sx={{ flex: 1, minWidth: { xs: 70, sm: 80 }, p: 1, borderRadius: 1.5, bgcolor: alpha(h.gain >= 0 ? cardSuccess : cardError, isDark ? 0.1 : 0.06), overflow: "hidden" }}>
              <Typography sx={{ fontSize: "0.6rem", fontWeight: 500, color: cardMuted, textTransform: "uppercase", letterSpacing: "0.04em", mb: 0.25 }}>
                P&L
              </Typography>
              <Typography noWrap sx={{ fontSize: "0.8rem", fontWeight: 700, color: h.gain >= 0 ? cardSuccess : cardError }}>
                {h.gain >= 0 ? "+" : ""}{fmt(h.gain, h.displayCurrency)}
              </Typography>
              <Typography sx={{ fontSize: "0.65rem", fontWeight: 600, color: h.gain >= 0 ? cardSuccess : cardError, opacity: 0.8 }}>
                {h.gain >= 0 ? "+" : ""}{gainPct.toFixed(1)}%
              </Typography>
            </Box>
          )}
          <Box sx={{ flex: 1, minWidth: { xs: 70, sm: 80 }, p: 1, borderRadius: 1.5, bgcolor: alpha(h.dayChange >= 0 ? cardSuccess : cardError, isDark ? 0.1 : 0.06), overflow: "hidden" }}>
            <Typography sx={{ fontSize: "0.6rem", fontWeight: 500, color: cardMuted, textTransform: "uppercase", letterSpacing: "0.04em", mb: 0.25 }}>
              Today
            </Typography>
            <Typography noWrap sx={{ fontSize: "0.8rem", fontWeight: 700, color: h.dayChange >= 0 ? cardSuccess : cardError }}>
              {h.dayChange >= 0 ? "+" : ""}{fmt(h.dayChange, h.displayCurrency)}
            </Typography>
            <Typography sx={{ fontSize: "0.65rem", fontWeight: 600, color: h.dayChange >= 0 ? cardSuccess : cardError, opacity: 0.8 }}>
              {h.dayChange >= 0 ? "+" : ""}{dayPct.toFixed(2)}%
            </Typography>
          </Box>
        </Stack>
      </Paper>
...
const TxnCard = memo(function TxnCard({ row, index, showInvested, canDelete, onDelete }: {
  row: TxnRow;
  index: number;
  ...
  const { txn: t, dt, delta, investedDelta, isLast } = row;
  const { colors, shadow } = useTokens();
  const isAdd = delta >= 0;
      ...
      <Box sx={{ display: "flex", gap: { xs: 1, sm: 2.5 }, alignItems: "flex-start", position: "relative" }}>
        ...
        <Paper sx={{
          ...
          <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
            ...
            {canDelete && (<IconButton size="small" onClick={() => onDelete(t)} sx={{ color: colors.error, opacity: 0.6, "&:hover": { opacity: 1, bgcolor: alpha(colors.error, 0.08) }, mt: -0.5 }}>
              <DeleteOutlineIcon sx={{ fontSize: 16 }} />
            </IconButton>)}
          </Stack>
        </Paper>
...
function AccountDetail() {
  ...
  const [syncCurrency, setSyncCurrency] = useState(DEFAULT_CURRENCY);
  ...
  // The account row and its watchlist chips come from independent endpoints, so they go out
  // together instead of in two effects that each waited for their own render.
  useEffect(() => {
    if (!userId || !numAccountId) return;
    let cancelled = false;
    setLoading(true); setError(null);
    Promise.all([
      getAccounts(userId),
      getAccountWatchlists(numAccountId).catch(() => [] as WatchlistSummary[]),
    ])
      .then(([accounts, wls]) => {
        if (cancelled) return;
  ...
  }, [userId, numAccountId, dataVersion]);

  // Gated on `account` because its type decides which of the two lists this page shows.
  useEffect(() => {
    if (!account) return;
    let cancelled = false;
    if (isBroker) {
      ...
      Promise.all([
        ...
        .catch(err => { if (!cancelled) showToast(err instanceof Error ? err.message : "Failed to load transactions", "error"); })
  ...
  const handleCreateHoldingFromQuote = async (quote: YahooQuote) => {
    ...
    const optimistic: HoldingSummary = {
      id: tempId, accountId: numAccountId, name, symbol, units: 0, unitsAreShares: true,
      currentDayValue: 0, previousDayValue: 0, invested: 0, gain: 0, dayChange: 0,
      xirr: null, displayCurrency: account?.displayCurrency ?? preferredCurrency,
    };
  ...
  const handleCreateTxn = async () => {
    ...
    const invested = showInvested ? parseFloat(parseFloat(txnInvested).toFixed(2)) : parseFloat(parseFloat(txnValue).toFixed(2));
    const value = parseFloat(parseFloat(txnValue).toFixed(2));
    const date = txnDate;
    const holdingId = defaultHoldingId;
    const prev = transactions;
    const tempId = -Date.now();
    const last = transactions[0];
    // Optimistic only: mirrors the row the server will return, including its running total and
    // its units-vs-money rule (only BROKER stores `value` as units; `showInvested` also covers
    // needsDailyData accounts, whose `value` is money). The server's row replaces this.
    const optimistic: Transaction = {
      id: tempId, accountId: numAccountId, holdingId, txnDate: date,
      invested: txnMode === "add" && last ? invested + last.invested : invested,
      value: txnMode === "add" && last ? value + last.value : value,
      displayCurrency: last?.displayCurrency ?? account?.currency ?? preferredCurrency,
      valueInUnits: account?.type === "BROKER",
    };
    setTransactions(t => [optimistic, ...t]);
    setCreateTxnOpen(false); setTxnDate(""); setTxnInvested(""); setTxnValue(""); setTxnMode("add");
    try {
      await createTransaction({ accountId: numAccountId, holdingId, txnDate: date, invested, value, mode: txnMode });
      invalidateMoneyCaches();
  ...
  const openWlDialog = async () => {
    ...
    try {
      // The linked set is already on screen as chips — only the full list is missing.
      const all = await getWatchlists(userId);
      setAllWatchlists(all.filter(w => w.name !== "All"));
      setLinkedWlIds(new Set(accountWatchlists.map(w => w.id)));
  ...
  const handleSyncMf = async () => {
    ...
    try {
      ...
      setSyncCurrency(result.currency);
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      setSyncDialogOpen(false);
      setSyncDiffs([]);
      // The sync writes holdings and transactions, so every money value on screen is stale.
      refreshAll();
    } catch (err) {
  ...
  const holdingRows = useMemo<HoldingRow[]>(() => holdings.map(h => ({
    h,
    gainPct: h.invested > 0 ? (h.gain / h.invested) * 100 : 0,
    dayPct: h.previousDayValue > 0 ? (h.dayChange / h.previousDayValue) * 100 : 0,
  })), [holdings]);

  // Each row's deltas come from the next (older) transaction, so one pass over the list replaces
  // the per-render index lookups the ledger used to do inside its map.
  const txnRows = useMemo<TxnRow[]>(() => transactions.map((t, i) => {
    ...
    return {
      ...
      delta: t.valueDelta ?? t.value - (older?.value ?? 0),
      investedDelta: t.investedDelta ?? t.invested - (older?.invested ?? 0),
      isLast: i === transactions.length - 1,
    };
  }), [transactions]);

  const { allSynced, changedDiffs, unchangedDiffs } = useMemo(() => ({
    allSynced: syncDiffs.length > 0 && syncDiffs.every(d => d.status === "UNCHANGED"),
    changedDiffs: syncDiffs.filter(d => d.status !== "UNCHANGED"),
  ...
  const acctDayPct = account && account.previousDayValue > 0 ? (account.dayChange / account.previousDayValue) * 100 : 0;
  // XIRR is only meaningful for accounts that track what was invested.
  const acctXirr = showInvested ? account?.xirr ?? null : null;
  const stockSyncPl = stockSyncPreview ? stockSyncPreview.value - stockSyncPreview.invested : 0;
    ...
    <Stack spacing={{ xs: 2.5, sm: 3 }}>
      ...
      {loading ? <ListSkeleton rows={3} /> : account && (
        <>
              ...
              <Paper sx={{
                ...
                <Box sx={{ px: 2, py: 1.5, borderRadius: 2, bgcolor: heroSubtle, display: "inline-block" }}>
                  ...
                  <Typography sx={{ fontSize: { xs: "1.75rem", sm: "2.25rem" }, fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1.1, color: heroText }}>
                    {fmt(account.currentDayValue, account.displayCurrency)}
                  </Typography>
                </Box>
                  ...
                  {showInvested && (
                    <Box sx={{ flex: "1 1 auto", minWidth: { xs: "calc(50% - 6px)", sm: 120 }, p: { xs: 1, sm: 1.5 }, borderRadius: 2, bgcolor: alpha(heroInvested, isDark ? 0.1 : 0.06) }}>
                      ...
                      <Typography sx={{ fontSize: "0.95rem", fontWeight: 700, color: heroInvested }}>
                        {fmt(account.invested, account.displayCurrency)}
                      </Typography>
                    </Box>
                  )}
                  {showInvested && account.invested > 0 && (
                    <Box sx={{ flex: "1 1 auto", minWidth: { xs: "calc(50% - 6px)", sm: 120 }, p: { xs: 1, sm: 1.5 }, borderRadius: 2, bgcolor: alpha(account.gain >= 0 ? heroSuccess : heroError, isDark ? 0.1 : 0.06) }}>
                      <Typography sx={{ fontSize: { xs: "0.6rem", sm: "0.65rem" }, fontWeight: 500, color: heroMuted, textTransform: "uppercase", letterSpacing: "0.04em", mb: 0.5 }}>
                        Total P&L
                      </Typography>
                      <Typography noWrap sx={{ fontSize: { xs: "0.75rem", sm: "0.95rem" }, fontWeight: 700, color: account.gain >= 0 ? heroSuccess : heroError }}>
                        {account.gain >= 0 ? "+" : ""}{fmt(account.gain, account.displayCurrency)}
                      </Typography>
                      <Typography sx={{ fontSize: "0.65rem", fontWeight: 600, color: account.gain >= 0 ? heroSuccess : heroError, opacity: 0.8 }}>
                        {acctGainPct >= 0 ? "+" : ""}{acctGainPct.toFixed(1)}%
                      </Typography>
                    </Box>
                  )}
                  <Box sx={{ flex: "1 1 auto", minWidth: { xs: "calc(50% - 6px)", sm: 120 }, p: { xs: 1, sm: 1.5 }, borderRadius: 2, bgcolor: alpha(account.dayChange >= 0 ? heroSuccess : heroError, isDark ? 0.1 : 0.06) }}>
                    <Typography sx={{ fontSize: { xs: "0.6rem", sm: "0.65rem" }, fontWeight: 500, color: heroMuted, textTransform: "uppercase", letterSpacing: "0.04em", mb: 0.5 }}>
                      Today
                    </Typography>
                    <Typography noWrap sx={{ fontSize: { xs: "0.75rem", sm: "0.95rem" }, fontWeight: 700, color: account.dayChange >= 0 ? heroSuccess : heroError }}>
                      {account.dayChange >= 0 ? "+" : ""}{fmt(account.dayChange, account.displayCurrency)}
                    </Typography>
                    <Typography sx={{ fontSize: "0.65rem", fontWeight: 600, color: account.dayChange >= 0 ? heroSuccess : heroError, opacity: 0.8 }}>
          ...
          {isBroker && (
            <>
              {holdingsLoading ? <ListSkeleton rows={4} /> : holdingRows.length === 0 ? (
                <Paper>
                  <EmptyState
                    icon={<ShowChartIcon />}
                    title="No holdings"
          ...
          {!isBroker && (
            <>
              {txnLoading ? <ListSkeleton rows={4} /> : txnRows.length === 0 ? (
                <Paper>
                  <EmptyState
                ...
                <>
                  <Typography variant="subtitle1" sx={{ color: colors.gray500 }}>
                    {txnRows.length} transaction{txnRows.length !== 1 ? "s" : ""}
                  </Typography>
                  <Stack spacing={0}>
                    {txnRows.map((row, i) => (
                      <TxnCard
                  ...
                  <Paper key={d.isin} variant="outlined" sx={{ p: 2, borderRadius: 2.5, borderColor: d.status === "NEW" ? alpha(colors.accent, 0.3) : alpha(colors.warning || "#FFA726", 0.3) }}>
                    ...
                    <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1.5 }}>
                      ...
                      <Box sx={{ p: 1.25, borderRadius: 1.5, bgcolor: isDark ? alpha(colors.pureWhite, 0.04) : colors.gray100 }}>
                        ...
                        {d.status === "NEW" ? (
                          <Typography sx={{ fontSize: "0.9rem", fontWeight: 700 }}>{fmt(d.kiteInvested, syncCurrency)}</Typography>
                        ) : (
                          <Stack direction="row" alignItems="center" spacing={0.5}>
                            <Typography sx={{ fontSize: "0.85rem", fontWeight: 600, color: colors.gray400 }}>{fmt(d.dbInvested, syncCurrency)}</Typography>
                            <ArrowForwardRoundedIcon sx={{ fontSize: 12, color: colors.gray400 }} />
                            <Typography sx={{ fontSize: "0.85rem", fontWeight: 700 }}>{fmt(d.kiteInvested, syncCurrency)}</Typography>
                          </Stack>
                        )}
                        {d.investedDiff !== 0 && (
                          <Typography sx={{ fontSize: "0.7rem", fontWeight: 600, mt: 0.25, color: d.investedDiff > 0 ? colors.success : colors.error }}>
                            {d.investedDiff > 0 ? "+" : ""}{fmt(d.investedDiff, syncCurrency)}
                          </Typography>
                        )}
                      </Box>
                    </Box>
            ...
            <Stack spacing={2}>
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 2 }}>
                <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, textAlign: "center" }}>
                  ...
                  <Typography sx={{ fontSize: "1.1rem", fontWeight: 700 }}>{fmt(stockSyncPreview.invested, stockSyncPreview.currency)}</Typography>
                </Paper>
                <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, textAlign: "center" }}>
                  ...
                  <Typography sx={{ fontSize: "1.1rem", fontWeight: 700 }}>{fmt(stockSyncPreview.value, stockSyncPreview.currency)}</Typography>
                </Paper>
              </Box>
              {stockSyncPl !== 0 && (
                <Paper variant="outlined" sx={{ p: 1.5, borderRadius: 2, textAlign: "center", bgcolor: stockSyncPl > 0 ? alpha(colors.success, 0.06) : alpha(colors.error, 0.06) }}>
                  <Typography sx={{ fontSize: "0.75rem", fontWeight: 600, color: stockSyncPl > 0 ? colors.success : colors.error }}>
                    P&L: {fmt(stockSyncPl, stockSyncPreview.currency)} ({(stockSyncPl / stockSyncPreview.invested * 100).toFixed(2)}%)
                  </Typography>
                </Paper>
              )}
            </Stack>
